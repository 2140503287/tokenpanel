...
function pem(value?: string): string {
  if (!value) throw new Error("missing Alipay key configuration");
...
function signContent(params: Record<string, string>): string {
  return Object.keys(params).filter((k) => k !== "sign" && k !== "sign_type" && params[k] !== "").sort().map((k) => `${k}=${encode(params[k]!)}`).join("&");
}
...
function rsaVerify(content: string, signature: string, publicKey: string): boolean {
...
function parseForm(input: string): Record<string, string> {
  const out: Record<string, string> = {};
  for (const [k, v] of new URLSearchParams(input)) out[k] = v;
  return out;
}
...
export class AlipayProvider implements PaymentProvider {
  ...
  async createPayment(order: RechargeOrder): Promise<PaymentCreateResult> {
    ...
    const res = await fetch(`${this.cfg.gatewayUrl ?? "https://openapi.alipay.com"}/gateway.do`, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded;charset=utf-8" }, body: form });
    const json = await res.json() as { alipay_trade_precreate_response?: { code?: string; msg?: string; qr_code?: string }; sign?: string };
  ...
  async verifyNotification(input: { body: string; headers: Record<string, string | undefined>; query: Record<string, string | undefined> }): Promise<PaymentNotifyResult> {
    ...
    const verified = rsaVerify(signContent(data), data.sign, this.cfg.alipayPublicKeyPem);
    if (!verified) return { verified: false, success: false };
